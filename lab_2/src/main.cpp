...
double potential(double id)
{
    return 1 / (1 + pow((0.01 * id), 2));
}
...
int main()
{
    ...
    while (true) {
        ...
        double id_cat { id(cats, obj) };
        double id_dog { id(dogs, obj) };

        cout << format("Cat's id number is (similarity with the ethalon): {}", id_cat) << endl;
        cout << format("Dog's id number is (similarity with the ethalon): {}", id_dog) << endl;

        double p_cat { potential(id_cat) };
        double p_dog { potential(id_dog) };

        cout << format("The potential to be a cat is: {}", p_cat) << endl;
        cout << format("The potential to be a dog is: {}", p_dog) << endl;
